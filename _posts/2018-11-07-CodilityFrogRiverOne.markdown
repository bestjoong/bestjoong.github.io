...
- [풀이]1) A배열을 처음부터 조회하며, 1 ~ X 범위에 있는 숫자를 발견되면 발견값을 저장한다.    첫 발견시, 증가량을 체크하는 변수를 ++ 해준다.2) 증가량을 체크하는 변수가 X와 같아지는 순간의 A배열의 인덱스값을 리턴해준다.3) A배열을 다 조회시에도 조건을 만족하지 못하면 -1을 리턴한다. 
